...
> **⚠️ Структура папок (рефактор):** весь бэкенд переехал из корня в пакет **`tis/`**.
> Точки входа `web_app.py` и `bot.py` остались в корне (Flask `template_folder`/`static_folder` — относительно корня). Раскладка пакета:
> `tis/engine.py`; `tis/core/` (config, net, data_cache, serialization, formatter, markets);
> `tis/data/` (market_data, data_fetcher, yfinance_provider, moex_provider, universe, instruments_catalog);
> `tis/analysis/` (analyzer, market_structure, fibonacci, deep_analysis, scalping, entry_advisor, market_reasons, accuracy_estimator, position_calculator, risk_manager, entry_zones, imbalance, candle_patterns);
> `tis/features/` (movers/screener/correlation/portfolio/fundamentals/fred/insider/news_provider, signal_journal);
> `tis/ai/` (ai_news, ai_personas).
> Импорты — абсолютные `from tis.<подпакет>.<модуль> import …`. **В тексте ниже модули названы плоско (`market_data.py` и т.п.) — фактический путь см. в раскладке выше.**
> Исследовательский инструмент insider-анализа вынесен в **`research/`** (`insider_backtest.py` + CSV + `insider_report.html`) — полностью автономен, удаляется без последствий.
> **Security-заголовки:** `@app.after_request` в `web_app.py` ставит CSP (под unpkg/Google Fonts/TradingView, `'unsafe-inline'` т.к. сборщика нет), X-Content-Type-Options, X-Frame-Options, Referrer-Policy, Permissions-Policy; HSTS — только под HTTPS.
...
- Фичи: `entry_zones.py` (зоны входа per-TF; **если обычных лонг/шорт-зон нет (напр. сильный тренд) — даёт ПРИБЛИЗИТЕЛЬНУЮ зону от объёма: `_volume_support`/`_volume_resistance` (Volume POC ниже/выше цены) → помечается `approx:True`, label «(приблизит.): объёмная поддержка/сопротивление»; на графике рисуется своим цветом + пунктиром + «~». **Цвета линий входа (лонг/шорт/приблизит.) настраиваются пользователем: кнопка «🎨 Цвета» (`#btnColors`) → модалка `#colorsOverlay`, хранится в localStorage `chart_colors`, применяется через `TradingChart.setEntryColors()`**), `imbalance.py` (FVG per-TF), `candle_patterns.py` (свечные паттерны per-TF + `next_candle_outlook`), `movers_provider.py` (**мини-спарклайны: ряд `spark` в строке — для klines-пути из уже загруженных свечей, для крипто-дня догружается 1h только для отображаемых карточек `_attach_crypto_sparklines`. ⚠️ Крипто-день сначала пробует bulk-тикер Binance `fetch_all_tickers_24h`; при 451/блокировке — фолбэк на per-инструмент klines (Yahoo). Фронт рисует SVG `sparklineSvg()`**), `screener_provider.py` (сортируемая колонка «Балл»: **НЕ запускает полный анализ сам** — через `data_cache.peek()` берёт точный балл из кэша `analyze:{market}:{ID}`, ЕСЛИ инструмент уже открывали в Обзоре (тогда совпадает с Обзором), иначе лёгкий прокси `_consistency()` по 1 ТФ. ⚡ Раньше гонял `cached_analysis()` (полный analyze_pair ~5с) на каждый инструмент → ~65с; теперь только 1 лёгкий запрос свечей на инструмент, пул `_MAX_WORKERS=10` → ~7с. Цена: клик по строке больше НЕ прогрет (анализ считается при открытии). Результат скринера кэш 300с), `correlation_provider.py`, `portfolio_provider.py` (Sharpe/просадка), `fundamentals_provider.py` (yfinance .info + `fetch_dividend_info()` для калькулятора дивидендов: цена/дивиденд-на-акцию/доходность, эндпоинт `/api/dividends`), `fred_provider.py` (макро), `insider_provider.py` (**инсайдерский/«умные деньги» фактор для АКЦИЙ: US — живой SEC Form 4 через EDGAR (покупки топ-менеджмента/крупные/кластерные = бычий буст, продажи = лёгкий минус); РФ — доли инсайдеров/институционалов из yfinance `.info`; крипта/валюта — N/A. Кэш 6ч. Вклад `score_adj` (−8..+8) идёт в балл согласованности в `accuracy_estimator`. Сырьё/бэктест — автономный `insider_backtest.py` → `insider_trades.csv`**), `universe.py` (полный список Binance/MOEX для поиска).
...
## Навигация (UX)
**Левый сайдбар** (`nav.side-nav#mainNav`, re-skin под Stitch): бренд сверху + группа «Рынок» (Крипта/Акции/Валюта) + группа «Разделы» (Обзор рынка/Скринер/Портфель/Новости/Журнал), иконки + подписи. Кнопки сохраняют `class="nav-tab"` + `data-view`/`data-market` → драйвят прежний `switchView`. Раскладка: `.terminal-body .app` — грид `[250px sidebar | 1fr .app-main]` на всю высоту (`100vh`, `overflow:hidden`), `.app-main` — flex-колонка с внутренним скроллом (роль старого `.app`); топбар без логотипа (логотип уехал в сайдбар). Адаптив <920px: сайдбар сворачивается в горизонтальную полосу сверху (иконки без подписей). Стили — в `refresh-pro.css` (секция «САЙДБАР-НАВИГАЦИЯ»).
...
3. ✅ **РЕШЕНО:** журнал перенесён в localStorage (stateless-сервер), общего `signals.json` больше нет — многопользовательский режим безопасен.
4. **Тесты (67, было 29)** покрывают ядро: position_calculator, risk_manager, markets+market_data-роутинг, analyzer (индикаторы/тренд), market_structure, fibonacci — всё детерминированно, без сети. **CI:** `.github/workflows/ci.yml` гоняет `python -m pytest` на push/PR (matrix 3.11/3.12). Ещё без тестов: провайдеры-фичи (imbalance, screener, portfolio, correlation, fred, fundamentals, candle_patterns, movers, ai_*, universe) — они сетевые, нужны моки.
